...
void usage(void)
{
	...
	fprintf(stderr, "-w1      starts the watchdog\n");
	fprintf(stderr, "-ws      disable watchdog\n");
...
int main(int argc, char *argv[])
{
	...
	for(loop=1; loop<argc; loop++)
	{
		...
		else if (strcmp(argv[loop], "-w1") == 0)
		{
			if (start_tapping_the_watchdog_without_fork(path, device_id))
				return 1;
		}
		else if (strcmp(argv[loop], "-ws") == 0)
		{
			if (stop_tapping_the_watchdog(path, device_id))
				return 1;
		}
